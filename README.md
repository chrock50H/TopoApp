...
Grid-View should have 1:1 correspondance to for Topo-Nodes, and each Element from Flat List can be highlighted for each user individually, and still be displayed as a merged Setmerged per border for the RCC View. 
...
Backoffice Implementation probably continous in python, some other options might follow. Like Oracle Java 1.8 LTS Runtime or a idealerly a containerized Webassembly/Rust parsing-script together with a signed Template and a second Container (docker image) which can be exchanged and maintained by local IT. providing best possible Interoperability in a Sandboxed offline environment.
